...
//    格式化double 小数点后为0不保留,不为0保留小数
    public static String doubleTrans(Double num)
    {
        BigDecimal bd = new BigDecimal(round(num, 2).toString());
     ...
     * @param text 要判断的文本
     * */
    public static boolean isContainsSensitive(String text){
        File file = null;
        Resource resource = new ClassPathResource("txtTemplate/synthesize.txt");
        InputStream inputStream = null;
        try {
            inputStream = resource.getInputStream();
            file = File.createTempFile("synthesize", ".txt");
